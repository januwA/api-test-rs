...
const K_REQ_TABS: [&str; 3] = ["Params", "Headers", "Body"];
...
struct ApiTestApp {
    ...
    request_query: Vec<KeyValuePair>,
    request_header: Vec<KeyValuePair>,
    request_body_form: Vec<KeyValuePair>,
    request_body_form_data: Vec<KeyValuePair>,
...
impl Default for ApiTestApp {
    fn default() -> Self {
        Self {
            ...
            body_raw_type_idx: Default::default(),
            request_query: vec![KeyValuePair::default()],
            request_header: vec![KeyValuePair::default()],
            request_body_form: vec![KeyValuePair::default()],
            request_body_form_data: vec![KeyValuePair::default()],
        }
    }
}
...
impl ApiTestApp {
    fn http_send(&mut self) {
        ...
        let body_raw_type_idx = self.body_raw_type_idx;
        let body_raw = self.body_raw.clone();
...
impl ApiTestApp {
    fn tabs_panel(&mut self, ui: &mut Ui, ctx: &egui::Context) {
        ui.horizontal(|ui| {
            for (i, label) in K_REQ_TABS.iter().enumerate() {
                ui.selectable_value(&mut self.req_tab_idx, i, label.to_owned());
            }
        });
    }
    ...
    fn req_header_panel(&mut self, ui: &mut Ui, ctx: &egui::Context) {
        ui.vertical(|ui| {
            if ui.button("添加").clicked() {
                self.request_header.push(KeyValuePair::default());
            }
        });
        ...
        egui_extras::StripBuilder::new(ui)
            .size(egui_extras::Size::remainder().at_least(50.0).at_most(200.0))
            .vertical(|mut strip| {
                strip.cell(|ui| {
    ...
    fn req_query_panel(&mut self, ui: &mut Ui, ctx: &egui::Context) {
        ui.vertical(|ui| {
            if ui.button("添加").clicked() {
                self.request_query.push(KeyValuePair::default());
            }
        });
        ...
        egui_extras::StripBuilder::new(ui)
            .size(egui_extras::Size::remainder().at_least(50.0).at_most(200.0)) // for the table
            ...
            ui.group(|ui| {
                ui.horizontal(|ui| {
                    for (i, raw_type) in K_REQ_BODY_RAW_TYPES.iter().enumerate() {
                        ui.radio_value(&mut self.body_raw_type_idx, i, raw_type.to_owned());
                    }
                });
            });
            ui.text_edit_multiline(&mut self.body_raw);
        });
    }
    ...
    fn req_body_form_panel(&mut self, ui: &mut Ui, ctx: &egui::Context) {
        ui.vertical(|ui| {
            if ui.button("添加").clicked() {
                self.request_body_form.push(KeyValuePair::default());
            }
        });
    ...
    fn req_body_form_data_panel(&mut self, ui: &mut Ui, ctx: &egui::Context) {
        ui.vertical(|ui| {
            if ui.button("添加").clicked() {
                self.request_body_form_data.push(KeyValuePair::default());
            }
        });
